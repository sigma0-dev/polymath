...
macro_rules! groth16_prove_bench {
    ...
    // type Polymath = sigma0_polymath::Polymath<Bls12_381, MerlinFieldTranscript<Fr>>;
        ...
        for _ in 0..NUM_PROVE_REPETITIONS {
            let _ = Polymath::<$bench_pairing_engine, MerlinFieldTranscript<$bench_field>>::prove(&pk, c.clone(), rng).unwrap();
        }
...
macro_rules! groth16_verify_bench {
    ($bench_name:ident, $bench_field:ty, $bench_pairing_engine:ty) => {
        ...
        let (pk, vk) = Polymath::<$bench_pairing_engine, MerlinFieldTranscript<$bench_field>>::circuit_specific_setup(c, rng).unwrap();
        let proof = Polymath::<$bench_pairing_engine, MerlinFieldTranscript<$bench_field>>::prove(&pk, c.clone(), rng).unwrap();
        ...
        for _ in 0..NUM_VERIFY_REPETITIONS {
            let _ = Polymath::<$bench_pairing_engine, MerlinFieldTranscript<$bench_field>>::verify(&vk, &vec![v], &proof).unwrap();
        }
